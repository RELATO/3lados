<%@page import="br.com.relato.extranet.prospects.SearchRecords"%>
...
%>
	...
	<style>
		...
		.active-column-7 {width: 140px;}
		.active-column-8 {width: 140px;}
		.active-column-9 {width: 140px;}
		.active-column-10 {width: 140px;}
		.active-column-11 {width: 80px;}
...
String change(Object d){
	if ( d != null ){
		...
		if ( "s".equals(value.toLowerCase()) )
			return "Sim";
		else if ( "n".equals(value.toLowerCase()) )
			return "Não";
	}
...
<%
			int count = 0;
			List list = SearchRecords.execute(request.getParameter("action"),
			...
			while ( i.hasNext() ){
				...
				if ( i.hasNext() ){
%>
					["<%=row.get(0)%>","<%=checkNull(row.get(1))%>", "<%=checkNull(row.get(2))%>", "<%=checkNull(row.get(3))%>", "<%=checkNull(row.get(4))%>", "<%=checkNull(row.get(5))%>", "<%=checkNull(row.get(6))%>", "<%=checkNull(row.get(7))%>", "<%=checkNull(row.get(8))%>", "<%=checkNull(row.get(9))%>", "<%=checkNull(row.get(10))%>", "<%=checkNull(row.get(11))%>"],
<%
				}else{
%>
					["<%=row.get(0)%>","<%=checkNull(row.get(1))%>", "<%=checkNull(row.get(2))%>", "<%=checkNull(row.get(3))%>", "<%=checkNull(row.get(4))%>", "<%=checkNull(row.get(5))%>", "<%=checkNull(row.get(6))%>", "<%=checkNull(row.get(7))%>", "<%=checkNull(row.get(8))%>", "<%=checkNull(row.get(9))%>", "<%=checkNull(row.get(10))%>", "<%=checkNull(row.get(11))%>"]
...
%>
		...
		var myColumns = [
			"Id", "Nome", "Email", "Endereço", "Bairro", "Cidade", "Estado", "Pais", "Cep", "Telefone", "Empresa","Ativo"
		];
	</script>
	...
	<table width="770" cellpadding="0" cellspacing="0" align="left" style="padding-top: 5px; padding-bottom: 3px; padding-left: 5px; background-color: #EBEADB; border-top: 1px solid #000000;">
	  <tr>
		...
		<td width="280" style="border-left: 1px solid #000000;" align="center">
			<input type="button" onClick="parent.doOpen(2);" class="botao2" name="novo" value="Novo Usuário" onMouseOver="this.className='botaoover2';" onMouseOut="this.className='botao2';">
