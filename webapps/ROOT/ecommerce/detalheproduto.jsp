...
<% 			
	//List lista = produto.getListaProdutos();	
	//System.out.println("Rafael opcao "+lista);
	int id = Cast.toInt(null != request.getParameter("idiproduto")?request.getParameter("idiproduto"):-1);
	int cont = 1;
	int tipoprod = 0;
	int tipoprodold = 0;
	Produto p = produto.getProduto(id);
	if(-1 != p.getIdiproduto()){
...
%>			
		<form id="frmproduto<%=cont%>" name="frmproduto<%=cont%>" onsubmit="validaSubmit(this);" action="inserirCarrinho.jsp">
			...
			<input type="hidden" id="send_to_url" name="send_to_url" value="<%=request.getRequestURL()%>">
<% 
...
%>					
			<tr>
				...
				<td height="25" colspan="3" bgcolor="D4DEE8">
				    <div class="titulo" align="center"><font color="#FFFFFF"><%=(p.getNmstipoproduto()).toUpperCase()%></font></div>
				</td>
			</tr>				
<% 					
