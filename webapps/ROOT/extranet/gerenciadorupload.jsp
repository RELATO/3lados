...
function openUpload(){
	window.open("/extranet/upload.jsp", "Upload", "width=600, height=400, top=100, left=100, status=no");
}

function down(){
	var id = getId();
	window.open("/extranet/extranetdownload?id="+id+"&type=<%=request.getParameter("type")%>", "Upload", "width=600, height=400, status=no");
}

window.pesquisa = function(){
	var iframe = document.getElementById("grid");
	iframe.src = "/extranet/grid.jsp?action="+document.getElementById("operacao").value+
...
function deleta(){
	...
	if ( id != null && id.length != 0 ){
		...
		var iframe = document.getElementById("grid");
		iframe.src = "/extranet/grid.jsp?delete=true&id="+id;
	}else
  ...
  <table style="border: 1px solid #000000; margin: 5px; margin-top: 10px;" width="770" cellpadding="0" cellspacing="0" bgcolor="#EBEADB">
    ...
    <tr> 
      <td colspan="4" style="border-top: 1px solid #000000;" bgcolor="#EBEADB"> 
        <span > 
        <iframe id="grid" src="/extranet/grid.jsp" frameborder="0" width="100%" height="238"></iFrame>
        </span> </td>
    </tr>
  </table>
</form>
