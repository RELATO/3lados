...
doc._ns={lookupNamespaceURI:function(){
	return null
	}
};
...
Active.System.Object.create=function(){
	...
	obj.clone=function(){
		...
		Active.System.Format.create=function(){
			...
			obj.dataToText=function(data){var value=this.dataToValue(data);
		...
		Active.System.HTML.create=function(){
			...
			obj.getTag=function(){return this._tag};
			obj._tag="div";
			obj.init=function(){
				...
				try{
					...
					for(i=1;i<classes.length;i++){
						...
						if(typeof(value)=="function"){
							...
							this[param]=value;value=param}tmp+="active-"+name+"-"+value+" "
						}
					...
					html+=tmp;html+=">"+innerHTML+"</"+this._tag+">";
		...
		obj.defineModel=function(name){
			...
			this[definePropertyArray]=function(property,defaultValue){
				...
				var getTempArray=function(){
					...
					for(i=0;i<max;i++){a[i]=this[_getProperty](i)}return a
				};
	...
	Active.Formats.Date.create=function(){
		...
		var tokens={
			"hh":"this.digits[this.date.getUTCHours()]",":mm":"':'+this.digits[this.date.getUTCMinutes()]","mm:":"this.digits[this.date.getUTCMinutes()]+':'","ss":"this.digits[this.date.getUTCSeconds()]","dddd":"this.longWeekdays[this.date.getUTCDay()]","ddd":"this.shortWeekdays[this.date.getUTCDay()]","dd":"this.digits[this.date.getUTCDate()]","d":"this.date.getUTCDate()","mmmm":"this.longMonths[this.date.getUTCMonth()]","mmm":"this.shortMonths[this.date.getUTCMonth()]","mm":"this.digits[this.date.getUTCMonth()]","m":"this.date.getUTCMonth()","yyyy":"this.date.getUTCFullYear()","yy":"this.digits[this.date.getUTCFullYear()%100]"
		};
		var match="";
		for(i in tokens){
			if(typeof(i)=="string")
		...
		obj.setTextFormat=function(format){
			...
			format="if(isNaN(value))return this._textError;"+"this.date.setTime(value+this._textTimezoneOffset);"+("return '"+format+"'").replace(/(''\+|\+'')/g,"");
	...
	Active.Templates.Image.create=function(){
		...
		var image=new Active.HTML.SPAN;image.setClass("box","image");
		image.setClass("image",function(){
	...
	Active.Templates.Link.create=function(){
		var obj=this.prototype;obj.setTag("a");
	...
	Active.Templates.List.create=function(){
		...
		var getIndexProperty=function(property){
			return this.$owner.getDataProperty(property,0)
		};
		...
		obj.getItemTemplate=function(index,temp){
			...
			if(!temp.selected){
				...
				template.getIndexProperty=getIndexProperty;
			}
	...
	Active.Templates.Header.create=function(){
		...
		var div=new Active.HTML.DIV;div.setClass("box","resize");
		...
		obj.setEvent("onmousedown",function(){
			this.setClass("header","pressed");window.status="Sorting...";
	...
	Active.Templates.Scroll.create=function(){
		...
		var space=new Pane;
		data.setClass("scroll","data");top.setClass("scroll","top");
		...
		var adjustSize=function(){
			...
			if(data){
				if(data.scrollHeight){
					space.runtimeStyle.height=data.scrollHeight > data.offsetHeight?data.scrollHeight:0;space.runtimeStyle.width=data.scrollWidth > data.offsetWidth?data.scrollWidth:0;
					var y=scrollbars.clientHeight;
					var x=scrollbars.clientWidth;data.runtimeStyle.width=x;top.runtimeStyle.width=x;data.runtimeStyle.height=y;left.runtimeStyle.height=y;top.scrollLeft=data.scrollLeft;left.scrollTop=data.scrollTop;scrollbars.runtimeStyle.zIndex=0
				}
				else{
		...
		obj.defineRowPropertyArray("text",function(i){
			...
			obj.defineStatusProperty("text",function(){
				switch(this.getStatusProperty("code")){
					...
					case "nodata":return "Nenhum registro encontrado !!!";
					default:return ""
				}
			});
			...
			obj.setColumnHeaderHeight=function(height){
				...
				var startColumnResize=function(header){
					...
					var doResize=function(){
						var el=header.element();
						var sz=size+event.clientX - pos;el.style.width=sz < 5?5:sz;el=null
					};
					var endResize=function(){
					var el=header.element();
					if(typeof el.onmouseleave=="function"){
						el.onmouseleave()}el.detachEvent("onmousemove",doResize);
						el.detachEvent("onmouseup",endResize);
						el.detachEvent("onlosecapture",endResize);
						el.releaseCapture();
						var width=size+event.clientX - pos;if(width < 5){width=5}el.style.width=width;
				...
				obj.setSelectionIndex=function(index){
					...
					try{
						...
						if(data.element().scrollTop > row.element().offsetTop - padding){
							top=row.element().offsetTop - padding;left.element().scrollTop=top;data.element().scrollTop=top;scrollbars.element().scrollTop=top
						}
			...
			var setSelectionValues=obj.setSelectionValues;obj.setSelectionValues=function(array){
			...
			var selectMultipleRows=function(src){
				...
				this.setSelectionProperty("values",selection);setSelectionIndex.call(this,index);
			...
			var selectRangeOfRows=function(src)
				...
				var start=row1 > row2?row2:row1;var count=row1 > row2?row1 - row2:row2 - row1;
				var i,selection=[];
				for(i=0;i<=count;i++){
	...
	Active.HTTP.Request.create=function(){
		...
		obj.request=function(){
			...
			for(i=1;i<headers.length;i++){name=headers[i];value=this["_"+name+"Header"];
			if(typeof value=="function"){
				value=value()
			}
...
Active.Text.Table.create=function(){
	...
	obj.response=function(text){
		...
		this._data=table;_super.response.call(this)
	};
	obj._data=[];
...
Active.XML.Table.create=function(){
	...
	obj.response=function(xml){
		this.setXML(xml);_super.response.call(this)
	};
	obj.defineProperty("XML");
	obj.setXML=function(xml){
		if(!xml.nodeType){
			var s=""+xml;xml=window.ActiveXObject?new ActiveXObject("MSXML2.DOMDocument"):new XMLDocument;xml.loadXML(s)
		}
